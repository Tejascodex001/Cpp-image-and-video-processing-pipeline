...
Mat resize_image(const Mat& img, int x, int y){
    Mat resized;
    Size sized(x,y);
...
Mat crop_image(const Mat &img, int x, int y, int w, int h){
    Rect crp(x,y,w,h);
...
Mat draw_color_img(const Mat &img, int start, int end, int b, int g, int r){
...
Mat draw_gray_img(const Mat &img, int start, int end, int val){
